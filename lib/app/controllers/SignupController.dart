import 'dart:convert';
import 'package:diyaar/app/views/home_view.dart';
...
class SignupController extends GetxController {
  ...
  var countries = [
    ...
    {'name': 'Saint Kitts & Nevis', 'dialCode': '+1', 'flag': '🇰🇳'},
    {'name': 'Saint Lucia', 'dialCode': '+1', 'flag': '🇱🇨'},
    {'name': 'Saint Vincent & Grenadines', 'dialCode': '+1', 'flag': '🇻🇨'},
    {'name': 'Samoa', 'dialCode': '+685', 'flag': '🇼🇸'},
    {'name': 'San Marino', 'dialCode': '+378', 'flag': '🇸🇲'},
    {'name': 'Sao Tome & Principe', 'dialCode': '+239', 'flag': '🇸🇹'},
    ...
    {'name': 'Trinidad & Tobago', 'dialCode': '+1', 'flag': '🇹🇹'},
  ...
  var selectedCountry = '+252'.obs; // Default Somalia

  // Countries list here (same as before)...
  ...
  Future<void> signup() async {
    ...
    final fullPhone = "${selectedCountry.value}${phoneController.text}";
    ...
    try {
      ...
      final body = {
        "user": {
          ...
          "defaultOrganizationUnitId": 0, // spelling corrected
      ...
      // 2️⃣ Print request body for debugging
      print("Request Body: ${jsonEncode(body)}");

      // 3️⃣ Make HTTP POST request
      final response = await http.post(
        Uri.parse(
          "https://api.diyar.space/api/services/app/User/CreateOrUpdateUser",
        ),
        headers: {"Content-Type": "application/json"},
        body: jsonEncode(body),
      );

      // 4️⃣ Print status code & raw response
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          Get.snackbar(
            "Error",
            res['error'] ?? res['message'] ?? "Failed to register user",
          );
        } catch (e) {
          // JSON decode failed
          ...
          Get.snackbar(
            "Error",
            "Failed to decode server response: ${response.body}",
          );
        }
      }
    } catch (e) {
